...
instance FromJSON Cfg where
  parseJSON (Yaml.Object v) = Cfg
    <$> v .: "session-token"

  parseJSON invalid =
    prependFailure "parsing Cfg failed, "
...
parseCfgFile fp = handle handler $ do
    parseResult <- Yaml.decodeFileEither fp
    case parseResult of
      Left err -> do
        fgColor Ansi.Dull Ansi.Red
        printf "Error parsing %v as YAML:\n" fp
        putStrLn $ Yaml.prettyPrintParseException err
        useEmptyCfg      
      Right tbl -> case cfgToken tbl  of
        Nothing -> pure $ Cfg Nothing
        Just tok -> if tok /= "" then
          pure $ Cfg (Just tok)
        else do
          fgColor Ansi.Dull Ansi.Red
          printf "Illegal YAML format: `session-token` should be a non-empty string.\n"
          useEmptyCfg
  where
